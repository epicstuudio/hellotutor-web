...
const ICON_MAP: Record<string, React.ReactNode> = {
    ...
    'what-we-collect': <Database className="w-6 h-6" />,
    'how-we-use': <Settings className="w-6 h-6" />,
    'legal-bases': <Gavel className="w-6 h-6" />,
    'who-we-share': <Share2 className="w-6 h-6" />,
    'how-long-we-keep': <Clock className="w-6 h-6" />,
    security: <Lock className="w-6 h-6" />,
    'your-rights': <ShieldCheck className="w-6 h-6" />,
    'automated-decision-making': <Cpu className="w-6 h-6" />,
    'children-privacy': <Baby className="w-6 h-6" />,
...
function SectionCard({ section, isActive }: { section: PolicySection; isActive: boolean }) {
    const st = useTranslations('privacyPolicy');
    return (
        <div
            id={section.id}
            data-pp-section={section.id}
            className={cn(
...
function RightsGrid() {
    ...
    const rights = useMemo(
        () => [
            { icon: <Eye className="w-5 h-5" />, key: 'access' },
            { icon: <UserCog className="w-5 h-5" />, key: 'rectification' },
            { icon: <HardDrive className="w-5 h-5" />, key: 'erasure' },
            { icon: <Fingerprint className="w-5 h-5" />, key: 'portability' },
            { icon: <KeyRound className="w-5 h-5" />, key: 'withdraw' },
            { icon: <Shield className="w-5 h-5" />, key: 'restrict' },
            { icon: <Lock className="w-5 h-5" />, key: 'object' },
            { icon: <Cpu className="w-5 h-5" />, key: 'automated' },
        ],
        [],
    );
    ...
    return (
        ...
        >
            <h3 className="text-h5 text-content mb-5">{t('rightsGrid.title')}</h3>
...
export function PrivacyPolicyContent() {
    ...
    useEffect(() => {
        const sectionEls = document.querySelectorAll('[data-pp-section]');
            ...
            (entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        const id = entry.target.getAttribute('data-pp-section');
